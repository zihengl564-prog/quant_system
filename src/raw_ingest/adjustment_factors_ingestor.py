...
class AdjustmentFactorsIngestor:
    def __init__(self):
    ...
    def backfill(
        ...
        self.app_logger.info(
            f"[AdjustmentFactorsIngestor] 开始回填 adj_factor, start_date={start_date}, "
        ...
        if not trade_dates:
            self.app_logger.warning(
                "[AdjustmentFactorsIngestor] 未找到可用交易日，请先确认 std_calendar 已构建"
            )
            return {
                "total_rows": 0,
                "success_slices": [],
        ...
        for trade_date_iso in trade_dates:
            ...
            self.app_logger.info(
                f"[AdjustmentFactorsIngestor] 拉取 adj_factor, trade_date={trade_date}"
            )

            try:
                df = self.provider.get_adj_factor(trade_date=trade_date)

                if df.empty:
                    self.app_logger.warning(
                        f"[AdjustmentFactorsIngestor] adj_factor 无数据, trade_date={trade_date}"
                    )
                    empty_slices.append(
                        {
                ...
                self.app_logger.info(
                    f"[AdjustmentFactorsIngestor] adj_factor 回填完成, trade_date={trade_date}, "
                    f"写入/更新 {row_count} 行"
                )
                ...
                self.error_logger.error(
                    f"[AdjustmentFactorsIngestor] adj_factor 权限不可用，终止该接口回填: "
                ...
                self.error_logger.exception(
                    f"[AdjustmentFactorsIngestor] adj_factor 分片失败, trade_date={trade_date}, "
        ...
        self.app_logger.info(
            f"[AdjustmentFactorsIngestor] adj_factor 回填结束, total_rows={total_rows}, "
