...
class DailyQuotesIngestor:
    ...
    def backfill(
        ...
        self.app_logger.info(
            f"[DailyQuotesIngestor] 开始回填 daily, start_date={start_date}, "
        ...
        if not trade_dates:
            self.app_logger.warning(
                "[DailyQuotesIngestor] 未找到可用交易日，请先确认 std_calendar 已构建"
            )
            return {
                "total_rows": 0,
                "success_slices": [],
        ...
        for trade_date_iso in trade_dates:
            ...
            self.app_logger.info(
                f"[DailyQuotesIngestor] 拉取 daily, trade_date={trade_date}"
            )

            try:
                df = self.provider.get_daily(trade_date=trade_date)

                if df.empty:
                    self.app_logger.warning(
                        f"[DailyQuotesIngestor] daily 无数据, trade_date={trade_date}"
                    )
                    empty_slices.append(
                        {
                ...
                self.error_logger.exception(
                    f"[DailyQuotesIngestor] daily 分片失败, trade_date={trade_date}, "
        ...
        self.app_logger.info(
            f"[DailyQuotesIngestor] daily 回填结束, total_rows={total_rows}, "
